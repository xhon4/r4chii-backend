...
/// tower-governor's rate limiter keys by peer IP; every request needs a
/// distinct one so this test's several register/login calls don't trip the
/// limiter meant for real brute-force attempts.
...
/// Registers, verifies, and logs in one account, returning its id and token.
///
